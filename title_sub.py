...
def save_subtitles_to_files(results):
    base_dir = "subtitles"
    ...
    for i, result in enumerate(results, 1):
        if result['status'] == 'success' and result.get('subtitles'):
            subtitles_content = result['subtitles']

            if subtitles_content and subtitles_content not in [
                ...
                safe_title = safe_title[:80]  # tránh tên quá dài
    ...
    if saved_count > 0:
        print(f"\n{Colors.OKGREEN}✓ Saved {saved_count} videos into folder '{base_dir}'{Colors.ENDC}")
    else:
        print(f"\n{Colors.WARNING}⚠ No subtitle file was saved{Colors.ENDC}")


def save_results(results, output_file='youtube_results.json'):
    try:
        with open(output_file, 'w', encoding='utf-8') as f:
            json.dump(results, f, ensure_ascii=False, indent=2)
        print(f"{Colors.OKGREEN}✓ Đã lưu kết quả vào {output_file}{Colors.ENDC}")

        save_subtitles_to_files(results)
...
def process_urls(urls):
    ...
    for i, url in enumerate(urls):
        ...
        if result['status'] == 'success':
            ...
            print(f"  {Colors.FAIL}✗{Colors.ENDC} {result['error']}")

        time.sleep(0.5)

    progress_bar(total, total, "Hoàn thành")
    print(f"\n\n{Colors.OKGREEN}✓ Đã xử lý xong tất cả video!{Colors.ENDC}")
...
def display_results(results):
    ...
    for i, result in enumerate(results, 1):
        ...
        if result['status'] == 'success':
            ...
            print(f"  {Colors.FAIL}Lỗi:{Colors.ENDC} {result['error']}")
            print(f"  {Colors.FAIL}URL:{Colors.ENDC} {result['url']}")
    ...
    while True:
        ...
        try:
            ...
            if choice == '1':
                ...
                confirm = input(f"{Colors.OKCYAN}Tiếp tục xử lý? (y/N): {Colors.ENDC}")
